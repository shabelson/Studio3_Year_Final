...
public class GrasshopperInUnity : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Rhino.Runtime.HostUtils.RegisterNamedCallback("FromGHPline", FromGHPline);
    ...
    {
        var objs = Resources.FindObjectsOfTypeAll<GameObject>().Where(obj => obj.name.Contains(id));
    ...
    void FromGHPline(object sender, Rhino.Runtime.NamedParametersEventArgs args)
    {
        if (Application.isPlaying)
        {
            Rhino.Geometry.GeometryBase[] values;
            ...
            {
                args.TryGetColor("diffuse", out diffuse);
                args.TryGetGeometry("pline", out values);
